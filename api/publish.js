 ...
 * 사장님은 비밀번호 네 자리만 아시면 된다.
 *
 * 버셀에 넣어 두어야 하는 값 (Settings → Environment Variables)
 *   GITHUB_TOKEN   깃허브에서 받은 열쇠 (Contents: Read and write)
 *   GITHUB_REPO    계정명/저장소명   예) parkjiwoon-writer/yulyi-site
 *   ADMIN_PIN      관리자 비밀번호 네 자리 (기본 0497)
  ...
  try {
    ...
    if (body.images && typeof body.images === 'object') {
      for (const key of Object.keys(body.images)) {
        ...
        const path = `assets/img/cms/${safe}.${ext}`;
